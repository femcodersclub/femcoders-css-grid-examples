document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateItemCount() {
    ...
    // Actualizar el grid después de cambiar los elementos
    updateGrid();
  }
  ...
  function updateGridLines() {
    if (showLinesCheckbox.checked) {
      ...
      // Solo actualizamos el backgroundSize, no manipulamos otros estilos de grid
      // que podrían interferir con grid-auto-flow
...
gap: 10px;
  ...
  playgroundGrid.addEventListener("mouseover", function (e) {
    if (e.target.classList.contains("grid-item")) {
      ...
      const columnWidth =
        gridRect.width / parseInt(explicitColumnsSelect.value);
      const rowHeight = 100;

      const column =
        Math.floor((itemRect.left - gridRect.left) / columnWidth) + 1;
      ...
      tooltip.textContent = `Elemento ${item.textContent}: Grid ${
        isExplicit ? "Explícito" : "Implícito"
      } (Col ${column}, Fila ${row})`;
  ...
  // Inicializar el grid - Importante: hacerlo al final
  updateGrid();
});
